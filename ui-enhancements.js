...
function exposeBuildVersion(){
  document.querySelectorAll('[data-app-version],.version-badge,.about-version span').forEach(el=>{el.textContent=BUILD_VERSION});
}
async function refreshServiceWorker(){
  ...
  }catch(error){console.warn('MusicChatLab service-worker refresh',error)}
}
...
function recoverClab(){if(bindClabToolbar())return;if(window.__mclClabRecoveryLoading)return;window.__mclClabRecoveryLoading=true;const s=document.createElement('script');s.src='clab-document-v1.js?v=2.1.0-recovery';s.onload=()=>{window.__mclClabRecoveryLoading=false;setTimeout(bindClabToolbar,20)};s.onerror=()=>{window.__mclClabRecoveryLoading=false;const n=document.getElementById('composerNote');if(n)n.textContent='CLAB-Modul konnte nicht geladen werden. Bitte die Seite neu laden.'};document.head.appendChild(s)}
...
function startRecovery(){exposeBuildVersion();refreshServiceWorker();clearStaticComposerNote();recoverChatShell();recoverClab();setTimeout(recoverChatShell,250);setTimeout(recoverClab,300)}
