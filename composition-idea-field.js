(()=>{
'use strict';
if(window.__mclCompositionIdeaFieldV1316)return;
window.__mclCompositionIdeaFieldV1316=true;
...
function syncFromActive(){return false}
...
function start(){const box=input();if(!box)return;box.addEventListener('input',()=>saveDraft(box.value));restoreDraft();const messages=el('messages');if(messages)new MutationObserver(()=>setTimeout(syncLatestGenerated,0)).observe(messages,{childList:true});document.addEventListener('click',e=>{if(e.target?.closest?.('.chat-item,#newChatButton'))setTimeout(restoreDraft,30)},true)}
window.MCLCompositionIdea={version:'1.3.16',get,set,syncFromActive,restoreDraft,compactGenerated,activeChatId};
