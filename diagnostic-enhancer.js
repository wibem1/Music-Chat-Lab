...
window.MCLDownloadDiagnostic=async function(){await window.MCLSettingsStore?.persistCurrent?.();const data=buildDiagnostic(),name=`Music-Chat-Lab-Diagnose-${new Date().toISOString().replace(/[:.]/g,'-')}.json`,text=JSON.stringify(data,null,2),file=new File([text],name,{type:'text/plain'});if(navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file]});return}catch(e){if(e?.name==='AbortError')return}}const blob=new Blob([text],{type:'application/octet-stream'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1500)};
