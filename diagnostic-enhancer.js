...
async function buildDiagnostic(){const base=readJSON(DIAG_KEY,{});let requestedSlots=[];try{requestedSlots=window.MCLMidiRequestedSlots?.()||[]}catch{}let settingsStorage=null;try{settingsStorage=await window.MCLSettingsStore?.diagnostic?.()||null}catch{}return{...base,diagnosticFormat:4,appVersion:currentAppVersion(),capturedAt:new Date().toISOString(),settingsStorage,chatContext:chatSnapshot(),pendingCompositions:pendingSnapshot(),workspace:workspaceSnapshot(),autoRequestedSlots:requestedSlots}}
