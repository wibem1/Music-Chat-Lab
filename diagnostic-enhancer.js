...
function buildDiagnostic(){let requestedSlots=[];try{requestedSlots=window.MCLMidiRequestedSlots?.()||[]}catch{}return{diagnosticFormat:4,appVersion:currentAppVersion(),capturedAt:new Date().toISOString(),runtime:clone(window.MCLRequestRuntime?.lastDiagnostic?.()||null),chatContext:chatSnapshot(),pendingCompositions:pendingSnapshot(),workspace:workspaceSnapshot(),autoRequestedSlots:requestedSlots}}
