...
async function buildDiagnostic(){const lastRoute=readJSON(DIAG_KEY,{});let requestedSlots=[];try{requestedSlots=window.MCLMidiRequestedSlots?.()||[]}catch{}let settingsStorage=null;try{settingsStorage=await window.MCLSettingsStore?.diagnostic?.()||null}catch{}return{diagnosticFormat:7,appVersion:currentAppVersion(),capturedAt:new Date().toISOString(),settingsStorage,lastRouteDiagnostic:lastRoute&&Object.keys(lastRoute).length?lastRoute:null,chatContext:chatSnapshot(),pendingCompositions:pendingSnapshot(),workspace:workspaceSnapshot(),autoRequestedSlots:requestedSlots,aiCalls:window.MCLAiTrace?.snapshotCurrentChat?.()||window.MCLAiTrace?.snapshot?.()||[],playback:window.MCLPlaybackDiagnostic?.()||null}}
window.MCLDownloadDiagnostic=async function(){await window.MCLSettingsStore?.persistCurrent?.();const data=await buildDiagnostic(),name=`Music-Chat-Lab-Diagnose-${new Date().toISOString().replace(/[:.]/g,'-')}.json`,text=JSON.stringify(data,null,2),file=new File([text],name,{type:'text/plain'});if(navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file]});return}catch(e){if(e?.name==='AbortError')return}}const blob=new Blob([text],{type:'application/octet-stream'}),u=URL.createObjectURL(blob),a=document.createElement('a');a.href=u;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1500)};
