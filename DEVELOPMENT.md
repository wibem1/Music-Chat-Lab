...
> **ARCHITEKTURVORRANG:** Vor jeder Entwicklungsarbeit ist zuerst `ARCHITECTURE_CONTRACT.md` vollständig zu lesen. Diese Datei enthält die aktuell verbindlichen Architekturentscheidungen; `DEVELOPMENT.md` bleibt das chronologische Entwicklungs- und Fehlerprotokoll. Widerspricht eine ältere historische Notiz dem Architekturvertrag, darf die Historie nicht stillschweigend als aktuelle Vorgabe verwendet werden.
...
### v1.4.43 – verbindliche Trennung von Komposition und MIDI-Materialisierung
Ausgangspunkt war die Feststellung, dass v1.4.41 Claude im kombinierten Kompositions-/MCL_ACTION-Aufruf mit `thinking: disabled` ausführte. Der verworfene v1.4.42-Versuch, diesen kombinierten Aufruf lediglich auf adaptive/medium und 20.000 Tokens umzustellen, wurde nicht gemergt.

Vor der Implementierung wurde `ARCHITECTURE_CONTRACT.md` als verbindliche, von der chronologischen Historie getrennte Quelle angelegt. Der neue Datenweg liegt im bestehenden `session-orchestrator.js` und fügt kein weiteres Patch-Modul hinzu.

Der Komponiermodus besteht nun aus zwei klar getrennten Provider-Aufrufen. `musical_composition` ist die eigentliche musikalische Arbeit und erzeugt eine vollständige ABC-Partiturnotation in `<MCL_MUSIC>`. Diese Stufe kennt kein MCL_ACTION-/JSON-/MIDI-Pitch-Schema. Claude erhält hier adaptive Thinking mit high effort. `midi_materialization` erhält anschließend das fertige Manuskript und überträgt es ohne Neukomposition in die interne Partitur; nur hier wird Claude Thinking deaktiviert. Eine eventuell nötige `midi_repair`-Stufe bleibt ebenfalls rein technisch.
...
### v1.5.2/v1.5.3 – UI-Typografie: Kaskadenursache statt Override-Schicht
...
## 2026-09-26 – v1.9.3: Konsolidierung und reproduzierbare Regressionstests
- Quellprüfung fand zwei inkonsistente Initialisierungs-/Versionsstellen: Orchestrator prüfte `__mclSessionOrchestratorV151`, setzte aber `V150`; die PWA registrierte noch `service-worker.js?v=1.9.1` bei Cache/App v1.9.2. Beide im zuständigen Modul korrigiert; App, Cache und Registrierungs-URL auf 1.9.3 synchronisiert.
- Tatsächliche HTTP-Provider-Fehler behalten nun ihren ursprünglichen Status; lokale JSON-/Partiturfehler werden als technische Fehlertexte ohne erfundenen HTTP-500-Status zurückgegeben.
- Browser-Smoke erweitert: gültiges JSON, Kurzdezimalzahl `.5`, ausschließlich fehlende äußere Schlussklammer, tatsächlich abgeschnittenes JSON, Orchestrator-Guard und vollständiger Engine-Compose-Durchlauf mit simulierten Providerantworten inklusive MIDI-Bytes und Prüfung, dass keine unnötige Fortsetzung aufgerufen wird.
- Die zentrale Engine bleibt unverändert v2.2.3, weil die letzte Diagnose vor dieser Engine-Version entstand. Weitere Engine-Änderungen erst bei nachgewiesenem reproduzierbarem Fehler. GitHub-/Browser-Ergebnisse und echte Provider-/iPad-Prüfung getrennt bewerten.
