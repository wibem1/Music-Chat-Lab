...
## v2.1.0 – Konsolidierung nach Vollprüfung
Nach der Vollprüfung von v2.0.2 wird der Laufzeitpfad bereinigt statt weiter symptomatisch gepatcht. Die Diagnose übernimmt keinen historischen `last-diagnostic.v1`-Block mehr, sondern protokolliert den tatsächlich letzten Request der neuen `MCLRequestRuntime`. Das dynamische Nachladen von `model-extension.js` ist entfernt; `model-catalog.js` ist die einzige Modellquelle im Laufzeitpfad. Die alten Reparaturschichten `runtime-compat.js`, `composition-state.js` und `player-variants.js` werden nicht mehr von der App geladen. Der MIDI-Export schreibt nun die in v2.0.0 dokumentierten Tempo- und Taktwechsel `tm`/`tsm` als Standard-MIDI-Metaevents. Neue Regressionstests verhindern das erneute Einschleusen des Legacy-Modellmoduls, eines zweiten Modellkatalogs und historischer Diagnosezustände. Freigabe erst nach erfolgreichem vollständigem Actions-Testlauf und Pages-Deploy.
