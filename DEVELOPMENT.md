...
### v1.4.42 – Claude-Komposition: musikalisches Denken gezielt wiederhergestellt
Die Diagnose von v1.4.41 zeigte bei Claude Sonnet 5 einen objektiven Qualitäts-/Ausführungsfehler: Eine ausdrücklich auf 32 Takte angeforderte Fassung endete tatsächlich bereits nach 13 Takten. Gleichzeitig belegte der finale Provider-Request, dass im Komponiermodus weiterhin die mit v1.4.36 eingeführte Einstellung `thinking: disabled` aktiv war. Diese Einstellung hatte den damaligen 32k-Token-Sink verhindert, nahm Claude aber auch im eigentlichen direkten Kompositionsschritt jede adaptive Denkphase.

Vor der Änderung wurden diese Entwicklungsdokumentation, `PROMPT_ARCHITECTURE.md`, der aktuelle Orchestrator sowie die Entstehung von v1.4.29 und v1.4.36 erneut geprüft. Die in v1.4.29 bewusst verworfene Zwei-Stufen-Komposition wird **nicht** wieder eingeführt. Es gibt weiterhin genau einen direkten musikalischen Kompositionsschritt: dieselbe Claude-Instanz erfindet die tatsächlichen Noten und liefert anschließend innerhalb desselben Aufrufs die technische MCL_ACTION.

Die Korrektur erfolgt ausschließlich in der bestehenden Anthropic-Konfiguration des zuständigen Orchestrators: Claude erhält im Komponiermodus wieder adaptives Thinking, jedoch mit `effort:"medium"` und einem gemeinsamen Ausgabelimit von mindestens 20.000 Tokens. Damit wird weder ein vorgeschalteter Prosabauplan noch eine zusätzliche musikalische Promptregel eingeführt. Gegenüber v1.4.35 wird die frühere Kombination `effort:"high"` + 32.768 Tokens bewusst **nicht** wiederhergestellt, weil genau diese Kombination vollständig im Thinking enden konnte. Gegenüber v1.4.41 wird Thinking nicht mehr vollständig abgeschaltet.

Verbindliche Architekturregel: Provider-/Token-Konfiguration darf die direkte musikalische Erfindung nicht in eine bloße Serialisierungsaufgabe verwandeln. Änderungen an Thinking/Reasoning im Komponiermodus sind künftig als musikalisch relevante Architekturänderung zu behandeln und müssen gegen die Diagnosehistorie geprüft werden. Der Regressionstest verlangt für Claude im Komponiermodus nun `thinking.type="adaptive"`, `effort="medium"` und mindestens 20.000 Output-Tokens.

Diese Änderung wird zunächst ausschließlich auf einem Entwicklungszweig geprüft. Sie wird nicht als freigegebener Stand bezeichnet und nicht nach `main` übernommen, bevor Syntax-/Browser-Smoke-Test und der tatsächliche Claude-Kompositionsweg geprüft sind.
