...
function sharedEngine(){const choice=document.getElementById('compositionEngineSelect')?.value||'standard',engine=choice==='sound-first'?window.MCLCompositionEngines?.soundFirst:window.MCLCompositionEngines?.standard;if(!engine?.compose)throw new Error('Die gewählte Composition Engine ist nicht geladen.');return engine}
...
async function sharedCompose(input,init,provider,body,task){
  const engine=sharedEngine(),snapshot={visibleTask:task,provider,model:String(body.model||''),engine:document.getElementById('compositionEngineSelect')?.value||'standard'},key='';
