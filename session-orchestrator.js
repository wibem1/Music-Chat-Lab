...
function sharedEngine(){const choice=document.getElementById('compositionEngineSelect')?.value||'standard',engine=window.CompositionEngineCatalog?.[choice]||window.CompositionEngineCatalog?.standard||window.CompositionEngine;if(!engine?.compose)throw new Error('Die gewählte Composition Engine ist nicht geladen.');return engine}
...
function sharedStageBody(provider,model,prompt,stage){if(provider==='anthropic'){const b={model,max_tokens:32768,messages:[{role:'user',content:prompt}]};if(/^claude-(?:sonnet|opus)-5(?:$|-)/i.test(String(model||''))&&stage==='midi_translation')b.thinking={type:'disabled'};return b}if(provider==='openai')return{model,input:[{role:'user',content:[{type:'input_text',text:prompt}]}],store:false};return{contents:[{role:'user',parts:[{text:prompt}]}]}}
...
async function sharedCompose(input,init,provider,body,task){
  await window.CompositionEngineCatalogReady;const engine=sharedEngine(),snapshot={visibleTask:task,provider,model:String(body.model||''),engine:document.getElementById('compositionEngineSelect')?.value||'standard'},key='';
  ...
  const result=await engine.compose({snapshot,key,seriesId:null,runId:'MCL-'+Date.now().toString(36),now:()=>new Date().toISOString(),requestModel,usedTitles:usedCompositionTitles()});result.run.selectedEngine={choice:snapshot.engine,name:engine.name,version:engine.version};return result;
}
