...
function sharedEngine(){const engine=window.CompositionEngine;if(!engine?.compose||engine.version!=='2.2.0')throw new Error('Composition Engine 2.2 ist nicht geladen.');return engine}
...
async function sharedCompose(input,init,provider,body,task){
  const engine=sharedEngine(),snapshot={visibleTask:task,provider,model:String(body.model||''),engine:'engine-2.2'},key='';
  ...
  const result=await engine.compose({snapshot,key,seriesId:null,runId:'MCL-'+Date.now().toString(36),now:()=>new Date().toISOString(),requestModel,usedTitles:usedCompositionTitles()});result.run.selectedEngine={choice:'engine-2.2',name:engine.name,version:engine.version};return result;
}
