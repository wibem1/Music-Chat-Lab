...
async function sharedCompose(input,init,provider,body,task){
  ...
  const requestModel=async({promptText,stage})=>{const result=await runMinimalStage(input,init,provider,body,promptText,stage);if(!result.r.ok)throw new Error('API-Fehler '+result.r.status);if(!result.raw)throw new Error('Die KI hat in '+stage+' keine Textantwort geliefert.');return result.raw};
