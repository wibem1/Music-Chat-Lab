...
function sharedEngine(){if(!window.CompositionEngine?.compose)throw new Error('Die gemeinsame Composition Engine ist nicht geladen.');return window.CompositionEngine}
...
async function sharedCompose(input,init,provider,body,task){
  const engine=sharedEngine(),snapshot={visibleTask:task,provider,model:String(body.model||'')},key='';
  ...
  return engine.compose({snapshot,key,seriesId:null,runId:'MCL-'+Date.now().toString(36),now:()=>new Date().toISOString(),requestModel,usedTitles:usedCompositionTitles()});
}
