...
function minimalToMclScore(obj,assignment){const x=minimalScoreObject(obj);if(!x||!Array.isArray(x.tracks)||!Number.isFinite(Number(x.bpm)))return null;const ts=Array.isArray(x.timeSignature)?x.timeSignature:[4,4];const tr=x.tracks.map((t,i)=>({nm:String(t?.name||`Track ${i+1}`),ch:Math.max(0,Math.min(15,Number.isFinite(Number(t?.channel))?Number(t.channel):i%16)),pg:Math.max(0,Math.min(127,Number(t?.program)||0)),nt:(t?.notes||[]).filter(n=>Array.isArray(n)&&n.length>=4&&!((Number(n[1])<=0||Number(n[3])<=0)&&Number(n[2])===0)).map(n=>[Number(n[0]),Number(n[1]),Math.round(Number(n[2])),Math.round(Number(n[3])),i,0.9]),ct:[]}));return{ti:String(x.title||obj?.title||'Neue Komposition'),bpm:Number(x.bpm),ts:{n:Number(ts[0])||4,d:Number(ts[1])||4},k:String(x.key||''),sm:String(x.description||x.summary||assignment||'').trim().slice(0,500),tr}}
