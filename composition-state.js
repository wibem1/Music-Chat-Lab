...
function saveActiveClab(){const api=window.MCLMidiSlots;if(!api?.all)throw new Error('Der MIDI-Arbeitstisch ist nicht verfügbar.');const slot=activeSlotNumber();if(!slot)throw new Error('Kein Stück auf dem Arbeitstisch markiert.');const item=(api.all()||[]).find(x=>Number(x.slot)===slot&&x.score);if(!item)throw new Error('Der markierte Speicherplatz enthält keine Komposition.');const score=clone(item.score),title=clean(score.ti)||clean(item.name)||'Komposition',old=window.MCLCLAB?.getLoadedDocument?.()||null,description=clean(score.sm)||clean(old?.concept),assignment=currentIdea()||clean(old?.assignment);score.ti=title;if(description)score.sm=description;const same=!!old&&JSON.stringify(old.score)===JSON.stringify(score),pm=activeProviderModel(),doc={...(same&&old?clone(old):{}),format:'composition-lab-document',version:1,savedAt:Date.now()/1000-APPLE_EPOCH,title,score,concept:description,provider:pm.provider,model:pm.provider?pm.model:null,measures:scoreMeasures(score),meter:`${score.ts?.n||4}/${score.ts?.d||4}`,tempo:String(score.bpm||96),musicalKey:String(score.k||''),ensemble:ensemble(score),assignment:assignment||latestAssignment()};if(old&&!same){doc.sourceName=old.title||old.sourceName||null;doc.sourceScore=clone(old.score)}delete doc.midiData;delete doc.musicXMLData;delete doc.costUSD;delete doc.inputTokens;delete doc.outputTokens;download(JSON.stringify(doc,null,2),safeName(title)+'.clab');try{window.MCLCLAB?.applyDocument?.(doc,title+'.clab')}catch(_){}setNote(`CLAB gespeichert: ${title}. Gespeichert wurde Speicher ${slot}, die aktuell markierte Komposition.`)}
