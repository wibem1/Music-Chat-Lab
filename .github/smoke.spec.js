...
test('core ui', async ({ page }) => {
  ...
  await expect(page.locator('[data-app-version]')).toHaveText('v1.4.24');
  ...
  const provenance = await page.evaluate(() => {
    ...
    const withSummary=window.MCLSessionV136.materializeAction({type:'new_score',summary:'Aktuelle Fassung',score:fresh},[source],'');
    const withoutSummary=window.MCLSessionV136.materializeAction({type:'new_score',score:fresh},[source],'');
  ...
  const validation = await page.evaluate(() => {
    ...
    return {broken:window.MCLSessionV136.scoreIssues(broken,{bpm:88,bars:24,key:'a-Moll'}),good:window.MCLSessionV136.scoreIssues(good,{bpm:88,bars:24,key:'a-Moll'})};
  });
  expect(validation.broken.some(x=>x.includes('Leerstelle'))).toBeTruthy();
  ...
  const ideaState = await page.evaluate(() => {
    ...
    const c=window.MCLSessionV136.explicitConstraints();
    const score={ti:'Frei',bpm:66,ts:{n:4,d:4},k:'e-Moll',tr:[{nm:'Piano',nt:[[0,4,60,80,0,1],[4,4,64,80,0,1]]}]};
    return {c,issues:window.MCLSessionV136.scoreIssues(score,c),reason:window.MCLSessionV136.deviationNote({deviationReason:'Die langsamere Bewegung trägt den Spannungsbogen.'},score)};
  });
