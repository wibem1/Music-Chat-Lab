...
test('core ui', async ({ page }) => {
  ...
  await expect(page.locator('[data-app-version]')).toHaveText('v1.9.5');
  ...
  const sharedEngine = await page.evaluate(() => {
    ...
    const claudeDraft=api.makeRequest('anthropic','claude-sonnet-5',prompts.composition,'composition');
    const claudeScore=api.makeRequest('anthropic','claude-sonnet-5',prompts.midiTranslation,'composition.score');
    const openaiDraft=api.makeRequest('openai','gpt-5.6',prompts.composition,'composition');
    const googleDraft=api.makeRequest('google','gemini-3.1-pro-preview',prompts.composition,'composition');
  ...
  expect(sharedEngine.prompts.composition).toContain('Komponiere jetzt das verlangte Stück vollständig');
  expect(sharedEngine.prompts.composition).toContain('Denke nicht an MIDI-Codierung');
  expect(sharedEngine.technical).toContain('Nur valides JSON');
  expect(sharedEngine.technical).toContain('Jede klingende Note der fertigen Komposition genau einmal ausgeben');
  expect(sharedEngine.claudeDraft.body.max_tokens).toBe(32768);
  expect(sharedEngine.claudeDraft.body.messages[0].content).toContain('Komponiere jetzt das verlangte Stück vollständig');
  ...
  expect(sharedEngine.googleDraft.body.contents[0].parts[0].text).toContain('Komponiere jetzt das verlangte Stück vollständig');
