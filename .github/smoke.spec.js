...
test('core ui', async ({ page }) => {
  ...
  await expect(page.locator('[data-app-version]')).toHaveText('v1.4.22');
  ...
  const provenance = await page.evaluate(() => {
    ...
    const withSummary=window.MCLSessionV134.materializeAction({type:'new_score',summary:'Aktuelle Fassung',score:fresh},[source],'');
    const withoutSummary=window.MCLSessionV134.materializeAction({type:'new_score',score:fresh},[source],'');
  ...
  const validation = await page.evaluate(() => {
    ...
    return {broken:window.MCLSessionV134.scoreIssues(broken,{bpm:88,bars:24,key:'a-Moll'}),good:window.MCLSessionV134.scoreIssues(good,{bpm:88,bars:24,key:'a-Moll'})};
  });
